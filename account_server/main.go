...
func main() {
	...
	if ok, err := grpc_service.start_grpc_service(fmt.Sprintf("%s:9999", cfg_node_addr)); !ok {
		glog.Errorln(err)
		panic(1)
		return
	}
	...
	if ok, err := http_service.start_http_service(server_addr); !ok {
		glog.Errorln(err)
		return
	}
	...
	glog.V(common.Log_Info_Level_1).Infof("%s is running. [http_addr = %s, node_id = %s]\n", cfg_node_id, server_addr, cfg_node_id)
	fmt.Printf("%s is running. [http_addr = %s, node_id = %s]\n", cfg_node_id, server_addr, cfg_node_id)
	...
	http_service.stop_http_service()
}
